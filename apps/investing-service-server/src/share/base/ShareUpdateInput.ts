/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  IsOptional,
  ValidateNested,
  IsNumber,
} from "class-validator";
import { PortfolioUpdateManyWithoutSharesInput } from "./PortfolioUpdateManyWithoutSharesInput";
import { Type } from "class-transformer";
import { TransactionUpdateManyWithoutSharesInput } from "./TransactionUpdateManyWithoutSharesInput";

@InputType()
class ShareUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => PortfolioUpdateManyWithoutSharesInput,
  })
  @ValidateNested()
  @Type(() => PortfolioUpdateManyWithoutSharesInput)
  @IsOptional()
  @Field(() => PortfolioUpdateManyWithoutSharesInput, {
    nullable: true,
  })
  portfolios?: PortfolioUpdateManyWithoutSharesInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  price?: number | null;

  @ApiProperty({
    required: false,
    type: () => TransactionUpdateManyWithoutSharesInput,
  })
  @ValidateNested()
  @Type(() => TransactionUpdateManyWithoutSharesInput)
  @IsOptional()
  @Field(() => TransactionUpdateManyWithoutSharesInput, {
    nullable: true,
  })
  transactions?: TransactionUpdateManyWithoutSharesInput;
}

export { ShareUpdateInput as ShareUpdateInput };
